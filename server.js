...
const STATELESS_POLICIES = {
  L4_ROUND_ROBIN: "L4_ROUND_ROBIN",
  L4_LEAST_CONN: "L4_LEAST_CONN",
  L7_PATH: "L7_PATH",
};

const SPECIALIZED_PATHS = ["/api", "/images", "/admin"];
...
const MIN_PROCESSING_DELAY_MS = 100;
const MAX_PROCESSING_DELAY_MS = 30000;
const DEFAULT_FAST_PROCESSING_DELAY_MS = 800;
const DEFAULT_SLOW_PROCESSING_DELAY_MS = 5000;
let virtualNodesPerServer = 10;

let currentMode = MODES.STATELESS;
let statelessPolicy = STATELESS_POLICIES.L4_ROUND_ROBIN;
let l4Index = 0;
let requestSequence = 1;

function createServerState(input, previousState, index = 0) {
  const capacity = Number(input.capacity);
  const normalizedPath = sanitizeServerPath(input.path, index);
  const processingDelayMs = normalizeProcessingDelayMs(
    input.processingDelayMs,
    previousState
      ? previousState.processingDelayMs
      : getDefaultProcessingDelayMs(index),
  );
  return {
    id: input.id,
    name: input.name,
    ...
    droppedCount: previousState
      ? previousState.droppedCount || 0
  ...
  createServerState(
    {
      ...
      path: "/api",
      capacity: 12,
      processingDelayMs: DEFAULT_FAST_PROCESSING_DELAY_MS,
    },
    null,
    0,
  ),
  ...
  createServerState(
    {
      ...
      path: "/admin",
      capacity: 12,
      processingDelayMs: DEFAULT_SLOW_PROCESSING_DELAY_MS,
    },
    null,
    2,
  ),
...
function normalizeVirtualNodeCount(value) {
  return Math.max(
    MIN_VIRTUAL_NODES,
    Math.min(MAX_VIRTUAL_NODES, Number(value) || 10),
  );
}
...
function getServerIdForStatefulMode(
  mode,
  requestId,
  serverList,
  vnodeCount = virtualNodesPerServer,
) {
...
function sanitizeServerPath(value, index = 0) {
  if (typeof value === "string" && value.trim().length) {
    const normalized = sanitizePath(value);
    if (SPECIALIZED_PATHS.includes(normalized)) {
      return normalized;
    }
  }
  return SPECIALIZED_PATHS[index % SPECIALIZED_PATHS.length];
}

function getDefaultProcessingDelayMs(index = 0) {
  return index === 2
    ? DEFAULT_SLOW_PROCESSING_DELAY_MS
    : DEFAULT_FAST_PROCESSING_DELAY_MS;
}

function normalizeProcessingDelayMs(value, fallbackMs) {
  ...
  if (!Number.isFinite(parsed)) {
    return Math.max(
      ...
      Math.min(MAX_PROCESSING_DELAY_MS, Math.round(fallback)),
    );
  }
  return Math.max(
    MIN_PROCESSING_DELAY_MS,
    Math.min(MAX_PROCESSING_DELAY_MS, Math.round(parsed)),
  );
}

function isValidStatelessPolicy(value) {
  return (
    value === STATELESS_POLICIES.L4_ROUND_ROBIN ||
    value === STATELESS_POLICIES.L4_LEAST_CONN ||
    value === STATELESS_POLICIES.L7_PATH
  );
}

function isL4StatelessPolicy(value) {
  return (
    value === STATELESS_POLICIES.L4_ROUND_ROBIN ||
    value === STATELESS_POLICIES.L4_LEAST_CONN
  );
...
function normalizeServerInput(input, index) {
  ...
  return {
    id,
    name,
    path: sanitizeServerPath(input.path, index),
    capacity: Number(input.capacity) || undefined,
    processingDelayMs: Number(input.processingDelayMs),
    count: Number(input.count) || 0,
    droppedCount: Number(input.droppedCount) || 0,
  };
}
...
function toServerSnapshot(serverState) {
  ...
  return {
    ...
    processingDelayMs: normalizeProcessingDelayMs(
      serverState.processingDelayMs,
      DEFAULT_FAST_PROCESSING_DELAY_MS,
    ),
    count: serverState.count,
    droppedCount: serverState.droppedCount || 0,
...
function formatStatelessPolicyLabel(policy) {
  if (policy === STATELESS_POLICIES.L4_ROUND_ROBIN) return "L4 Round Robin";
  if (policy === STATELESS_POLICIES.L4_LEAST_CONN)
    return "L4 Least Connections";
  if (policy === STATELESS_POLICIES.L7_PATH) return "L7 Path-Aware";
  return String(policy || STATELESS_POLICIES.L4_ROUND_ROBIN);
}

function formatModeLabel() {
  if (currentMode === MODES.STATELESS) {
    return `Stateless (${formatStatelessPolicyLabel(statelessPolicy)})`;
  }
...
function getRecentUniqueKeys(limit = 40) {
  ...
  const out = [];
  for (
    let i = recentRequestKeys.length - 1;
...
function getMetricsSnapshot() {
  return {
    ...
    migrationCostComparison: {
      ...
      consistent: Number(
        avg(metrics.rehashHistory[MODES.CONSISTENT]).toFixed(2),
      ),
    },
...
function computeAffectedRanges(
  ...
  oldServers,
  newServers,
  oldVNodeCount = virtualNodesPerServer,
  newVNodeCount = virtualNodesPerServer,
) {
  if (mode === MODES.MOD_N) {
  ...
  const addedVNodes = newRing.filter(
    (node) => !oldVNodeKeys.has(node.vnodeKey),
  );
  const removedVNodes = oldRing.filter(
    (node) => !newVNodeKeys.has(node.vnodeKey),
  );
  const ranges = [];
...
function computeRehashImpact(
  ...
  for (const key of keys) {
    const oldServer = getServerIdForStatefulMode(
      mode,
      key,
      oldServers,
      oldVNodeCount,
    );
    const newServer = getServerIdForStatefulMode(
      mode,
      key,
      newServers,
      newVNodeCount,
  ...
  return {
    ...
    affectedRanges: computeAffectedRanges(
      mode,
      oldServers,
      newServers,
      oldVNodeCount,
...
function computeModeSwitchImpact(fromMode, toMode, topology) {
  if (
    !isStatefulMode(fromMode) ||
    !isStatefulMode(toMode) ||
    !topology.length
  ) {
    return null;
  }
...
function applyRehashEvent(rehashEvent) {
  ...
  if (
    rehashEvent.mode === MODES.MOD_N ||
    rehashEvent.mode === MODES.CONSISTENT
  ) {
...
function selectTargetServer(request) {
  ...
  if (currentMode === MODES.STATELESS) {
    if (statelessPolicy === STATELESS_POLICIES.L7_PATH) {
      const matchedServers = servers.filter((srv) => srv.path === request.path);
      const randomIndex = Math.floor(Math.random() * matchedServers.length);
      return {
        targetServer: matchedServers[randomIndex] || null,
        requestHash: hashToRing(request.requestId),
      };
    }

    if (statelessPolicy === STATELESS_POLICIES.L4_LEAST_CONN) {
      const minConnections = Math.min(
        ...servers.map((srv) => Number(srv.activeConnections) || 0),
      );
      const leastConnected = servers.filter(
        (srv) => (Number(srv.activeConnections) || 0) === minConnections,
      );
      const targetServer =
        leastConnected[l4Index % leastConnected.length] ||
        leastConnected[0] ||
        null;
...
io.on("connection", (socket) => {
  ...
  socket.on("switch_mode", (payload) => {
    ...
    if (
      requestedMode === MODES.STATELESS ||
      requestedMode === MODES.MOD_N ||
      requestedMode === MODES.CONSISTENT
    ) {
      currentMode = requestedMode;
    }

    if (payload && typeof payload.statelessPolicy === "string") {
      if (isValidStatelessPolicy(payload.statelessPolicy)) {
    ...
    const switchedMode =
      oldMode !== currentMode || oldPolicy !== statelessPolicy;
    if (switchedMode) {
      resetServerStatistics();
      const modeSwitchRehash = computeModeSwitchImpact(
        oldMode,
        currentMode,
        oldTopology,
      );
  ...
  socket.on("update_stateless_policy", (payload) => {
    if (!payload || typeof payload.policy !== "string") return;
    if (!isValidStatelessPolicy(payload.policy)) return;
    ...
    statelessPolicy = payload.policy;

    console.log(`Stateless policy updated to ${statelessPolicy}.`);
    io.emit("mode_changed", getModePayload());

    if (oldMode !== currentMode || oldPolicy !== statelessPolicy) {
      resetServerStatistics();
      const modeSwitchRehash = computeModeSwitchImpact(
        oldMode,
        currentMode,
  ...
  socket.on("update_servers", (newServers) => {
    ...
    servers = normalizedInput.map((srvInput, index) => {
      const previous = previousById.get(srvInput.id);
      return createServerState(srvInput, previous, index);
    });
    ...
    const reason =
      addedServers.length && !removedServers.length
        ? "server_added"
  ...
  socket.on("update_server_processing_time", (payload) => {
    if (!payload || typeof payload.id !== "string") return;

    const server = servers.find((srv) => srv.id === payload.id);
    if (!server) return;

    server.processingDelayMs = normalizeProcessingDelayMs(
      payload.processingDelayMs,
      server.processingDelayMs,
    );

    console.log(
      `Updated processing time for ${server.name} (${server.id}) to ${server.processingDelayMs}ms.`,
    );
  ...
  socket.on("send_request", (data) => {
    ...
    const droppedForL4Mismatch = Boolean(
      currentMode === MODES.STATELESS &&
      isL4StatelessPolicy(statelessPolicy) &&
      targetServer &&
      targetServer.path !== pathValue,
    );
    ...
    const migrated = Boolean(
      targetServer && previousServerId && previousServerId !== targetServer.id,
    );

    if (targetServer) {
      requestToServer.set(requestId, targetServer.id);

      if (droppedForL4Mismatch) {
        targetServer.droppedCount = (targetServer.droppedCount || 0) + 1;
      } else {
        targetServer.count = (targetServer.count || 0) + 1;
      }

      targetServer.activeConnections =
        (targetServer.activeConnections || 0) + 1;
      targetServer.recentRequestTimes.push(now);
      refreshServerLoad(targetServer, now);

      const processDelay = normalizeProcessingDelayMs(
        targetServer.processingDelayMs,
        getDefaultProcessingDelayMs(0),
      );

      setTimeout(() => {
        const liveServer = servers.find((srv) => srv.id === targetServer.id);
        if (!liveServer) return;
        liveServer.activeConnections = Math.max(
          0,
          (liveServer.activeConnections || 0) - 1,
        );
